...
class SearchAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.tvProductPrice.text = listProduct[position].hargaRupiah.toString()
        holder.tvProductStore.text = listProduct[position].aroma

        holder.itemView.setOnClickListener{
            val intent = Intent(holder.itemView.context, DetailProduct::class.java )
            intent.putExtra("product_id", listProduct[position].id)
